...
| Path | For | Docs |
|---|---|---|
| **Docker Compose** (`make all`) | the default — one Linux host, each service its own container | this page |
| **Vexa Lite** (`make lite`) | the simplest — the whole control plane in **one container**, bots/agents as child processes | [Vexa Lite](/deployment/lite) |
| **Helm** (`deploy/helm/charts/vexa`) | Kubernetes — bots and agent workers spawn **as Pods** | [Kubernetes](/deployment/helm) |

All three run the same images and contracts; the release pipeline proves each from the published
images before a version ships.
