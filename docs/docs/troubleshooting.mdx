...
- **Check meeting-api's `/health` capability rows first**: `curl -s localhost:18080/health | jq
  .capabilities.stt`. `misconfigured` with a `401/403` probe reason = your token is **set but
  rejected** — the worst silent failure; fix the token, not the bots. See the
  [case study](/case-studies#1--the-rejected-stt-token--silent-transcript-less-meetings).
