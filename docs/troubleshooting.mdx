...
description: "Symptom-to-remedy guide for deployment, configuration, and transcription: bot joins, STT setup, model credentials, authentication."
---
...
## `POST /bots` returns 503 "STT service is not configured"

Fail-loud by design: a transcription bot without STT would only produce dead air, so the spawn
is rejected at the gate. Set `TRANSCRIPTION_SERVICE_URL` + `TRANSCRIPTION_SERVICE_TOKEN`
([Configuration](/configuration#transcription-stt)), or opt out explicitly
(`"transcribe_enabled": false` per spawn, or `TRANSCRIBE_ENABLED=false` deployment-wide) for
capture-only bots.

## Bot joins but there's no transcript

The bot is capturing audio, but transcription isn't working.

- **Check meeting-api's `/health` capability rows first**: `curl -s localhost:18080/health | jq
  .capabilities.stt`. `misconfigured` with a `401/403` probe reason means your
  `TRANSCRIPTION_SERVICE_TOKEN` is **set but rejected** by the STT endpoint — fix the token, not
  the bots. See [Operating principles → capabilities declare themselves](/principles#capabilities-declare-themselves).
...
## Chat fails with "Model inference error" / "Not logged in"

The agent worker spawned but has no model credential. Check `model_inference` on the runtime's
and agent-api's `/health` — `not_configured` means no credential path is set at all;
`misconfigured` with reason `directory` means `HOST_CLAUDE_CREDENTIALS` points at a missing host
path (Docker bind-mounted it as an empty directory). Fix per your deploy target — the three
mechanisms are in [Configuration → Model credentials](/configuration#model-credentials-on-each-deploy-target)
(docker: host credentials file bind-mount; k8s: Secret-fed env passthrough; lite: container env).

## `LLMConfigError: no completion endpoint`

The LLM provider selection is incomplete: `VEXA_LLM_PROVIDER=openai-compat` (the default) needs
`VEXA_LLM_BASE_URL` set to a completion endpoint, or switch provider (`VEXA_LLM_PROVIDER=anthropic`
or `claude-cli`). See [Configuration → `VEXA_LLM_PROVIDER`](/configuration#vexa_llm_provider).
