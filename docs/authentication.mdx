...
The gateway enforces scopes per route prefix; a key lacking the scope gets `403`:

| Route prefix | Accepted scopes |
|---|---|
| `/bots` | `bot` or `browser` |
| `/transcripts` | `tx` |
| `/meetings` | `tx` |
| `/recordings` | `tx` or `bot` |

So a bot-orchestration service can hold a `bot`-only key that can't read transcripts, and an
analytics consumer a `tx`-only key that can't send bots; mint `?scopes=bot,tx` for a client that
does both.
