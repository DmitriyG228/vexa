...
| `max_concurrent_bots=0` treated as **depleted, not unlimited** (main PR #456) | **carried** | A cap of `0` rejects the spawn as depleted (0.12 PR #44, mirroring main's #456), with a test asserting the behavior. |
| Swagger/OpenAPI **auth-scheme fixes** (main PRs #319, #336) | not carried | The 0.12 admin-api declares both `APIKeyHeader`s without `scheme_name`, and the 0.12 gateway declares no OpenAPI security schemes — a DX gap in the *served* Swagger docs (the sealed contract is unaffected). |
...
#### Reliability in 0.12.0

- **Durable transcript persistence** (#53) — transcripts persist durably to postgres and are
  available from REST during *and* after the meeting; the read path merges the live redis tail
  with the durable record; no schema migrations; plus durable processed-notes views and per-send
  session keying. See [Transcript persistence](/architecture/transcript-persistence).
- **Evidence-based meeting lifecycle** (#50) — the runtime re-adopts its workloads across
  restarts; an untracked-workload `404` is never treated as termination evidence; stops are
  confirmed; bots leave gracefully on SIGTERM
...
- **Lite agent worker** (#51) — the lite image ships a complete worker tree; process-backend
  worker output is captured per workload with an error tail on crash.
- **Worker model auth on k8s** (#52) — credentials reach worker Pods via an explicit
  dispatch-env allowlist + chart Secret wiring.
...
The design principles behind #49/#50/#53 are described in
...
- **Speaker attribution on Microsoft Teams and Zoom** — segments transcribe correctly but carry
  provisional `seg_N` speaker labels on Teams and Zoom in 0.12.0; fixed in **0.12.1**
...
## Breaking changes for 0.10 clients

0.12 validates 0.10 API compatibility behaviorally in CI (the `V010_COMPAT` suite runs on
every release build). Six responses changed shape; each is pinned by a strict test so no
further undocumented change can ship. Migration is a small client-side edit in every case —
the underlying behavior is unchanged.

| Call | 0.10 shape | 0.12 shape | Migration |
|---|---|---|---|
| `GET /bots/status` | `{"running_bots": [...]}` | `{"running": [...], "count": N}` | read `running` instead of `running_bots` |
| `DELETE /bots/{platform}/{id}` | `202` + `{"message"}` | `200` + `{"status": "stopping", ...}` | accept `200`; read `status` |
| Admin API via gateway | `/admin/*` proxied through the gateway | gateway returns 404; call the admin-api service directly | point admin tooling at the admin-api base URL |
| `PUT /user/webhook` | echoed the full user object | echoes `{id, email, name, max_concurrent_bots}`; the config is applied | `GET` after `PUT` if you need the applied config echoed |
| `/ws` connect | only subscribed frames, enveloped | also pushes flat `meeting.status` frames for your meetings on connect | ignore unknown frame shapes |
| `POST /admin/users`, `POST /admin/users/{id}/tokens` | included `created_at` (required in 0.10 client models) | `created_at` omitted | make the field optional in your models |

Also note: webhook signatures are now computed over `"<timestamp>." + payload`
(header `X-Webhook-Timestamp`; replay protection) — 0.10 receivers verifying
`hmac(payload)` must update their recipe ([webhooks guide](/how-to/webhooks)).
